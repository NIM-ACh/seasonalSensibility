...
tbl <- tbl_summary(data = datos[, .SD, .SDcols = genero:edad])
bold_labels(x = tbl)
```
:::
...
ggstatsplot::ggpiestats(datos, deporte_dias_semana,
  bf.message = F,
...
:::{#tab2}
```{r tab2, echo=FALSE}
tbl <- tbl_summary(data = datos[, .SD, .SDcols = deporte:deporte_minutos_sesion])
bold_labels(x = tbl)
```
:::
...
:::{#tab3}
```{r tab3, echo=FALSE}
tbl <- tbl_summary(data = datos[, .SD, .SDcols = ss_patron_verano:ss_severidad])
bold_labels(x = tbl)
```
:::
...
:::{#tab4}
```{r tab4, echo=FALSE}
tbl_summary(data = datos[, .SD, .SDcols = names(datos) %like% "riff"])
```
:::
